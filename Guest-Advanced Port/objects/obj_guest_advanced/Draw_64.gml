...
var _check_surf = function(_surf, _w, _h, _format) {
    if (!surface_exists(_surf)) return surface_create(_w, _h, _format);
    return _surf;
}
...
if (!surface_exists(surf_game)) {
    ...
    if (view_get_surface_id(0) != surf_game) view_set_surface_id(0, surf_game);
}
...
for (var i = 0; i < 2; i++) {
    ...
    surf_avglum[i]    = _check_surf(surf_avglum[i], 1, 1, surf_format);
}

// Intermediates (Resolutions from slangp)
...
// NTSC Chain (Resolution Fix applied)
...
// --- 3. Pipeline Execution Start ---

// CRITICAL: Disable blending for data processing.
// We want strict overwrite of RGBA values.
...
// Pass 6: NTSC Pass 2 (CHANGED TO NEAREST)
...
// --- 5. Final Output ---
// We can re-enable blending here if we want standard GUI behavior over the game,
// but for a pure post-process overwrite, keeping it off is slightly faster and safer.
...
// Re-enable blending for other GameMaker draw events
